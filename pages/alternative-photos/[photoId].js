...
const AlternativePhotoPage = ({photo}) => {
    const router = useRouter();

    if(router.isFallback) {
        return <h1>loading fallback...</h1>
    }
...
export async function getStaticProps(context) {
    ...
    // const response = await fetch(`http://127.0.0.1:4000/alternative-photos/${params.photoId}`);
    // const response = await fetch(`http://localhost:4000/alternative-photos/${params.photoId}`);
...
export default AlternativePhotoPage;